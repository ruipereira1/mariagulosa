...
// String de conexão do usuário (substitua <db_password> pela sua senha real)
const connectionString = 'mongodb+srv://ruivalentimefa:<db_password>@maria-gulosa.ghwdasr.mongodb.net/maria-gulosa?retryWrites=true&w=majority&appName=maria-gulosa'

// ⚠️ IMPORTANTE: Substitua <db_password> pela sua senha real antes de executar!

// Conectar ao MongoDB Atlas
const connectToAtlas = async (connStr) => {
  try {
    await mongoose.connect(connStr)
    console.log('✅ Conectado ao MongoDB Atlas com sucesso!')
    console.log(`📍 Conectado em: maria-gulosa.ghwdasr.mongodb.net`)
    return true
  } catch (error) {
    console.error('❌ Erro ao conectar:', error.message)
    console.log('\n💡 Dicas para resolver:')
    console.log('   1. Substitua <db_password> pela sua senha real')
    console.log('   2. Verifique se o IP está liberado (0.0.0.0/0)')
...
const main = async () => {
  ...
  if (connectionString.includes('<db_password>')) {
    console.log('❌ ERRO: Você precisa substituir <db_password> pela sua senha real!')
    console.log('')
    console.log('📝 INSTRUÇÕES:')
    console.log('1. Abra o arquivo: backend/configure-atlas.js')
    console.log('2. Na linha 4, substitua <db_password> pela sua senha do MongoDB Atlas')
    console.log('3. Salve o arquivo')
    console.log('4. Execute novamente: node configure-atlas.js')
    console.log('')
    console.log('💡 Sua string deve ficar assim:')
    console.log('mongodb+srv://ruivalentimefa:SUA_SENHA_AQUI@maria-gulosa.ghwdasr.mongodb.net/maria-gulosa?retryWrites=true&w=majority&appName=maria-gulosa')
    return
  }
